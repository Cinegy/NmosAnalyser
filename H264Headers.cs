...
namespace NmosAnalyser
{
    ...
    public unsafe struct H264HrdParams
    {
        ...
        public void Read(ref H264BitReader br)
        {
            ...
            fixed (uint* bit_rate = bit_rate_value, cpb_size = cpb_size_value) fixed (bool* cbr = cbr_flag)
            for (int idx = 0; idx < cpb_cnt; idx++)
            {
                bit_rate[idx] = br.Get_UE() + 1;
                cpb_size[idx] = br.Get_UE() + 1;
                cbr[idx] = br.Get_Bool();
            }
        ...
        public void Write(ref H264BitWriter bw)
        {
            ...
            for (int idx = 0; idx < cpb_cnt; idx++)
            {
                bw.Put_UE(bit_rate[idx] - 1);
                bw.Put_UE(cpb_size[idx] - 1);
                bw.Put_Bool(cbr[idx]);
            }
    ...
    public unsafe struct H264SeqParamSet
    {
        ...
        {
        	H264BitReader br = new H264BitReader(buf);
            ...
            else if (pic_order_cnt_type == 1)
            {
                ...
                fixed(int *p = offset_for_ref_frame)
        ...
        public byte[] Encode()
        {
        	byte [] bitstream = new byte[1024];
        	H264BitWriter bw = new H264BitWriter(bitstream);
            ...
            byte[] result = new byte[bw.BytesInBuffer];
        ...
        //-------------------------------------------------------------------------
        static unsafe void ReadScalingList(ref H264BitReader br, int id, byte* scaling_list, int list_size, bool* use_default_matrix)
        //-------------------------------------------------------------------------
        {
            byte[] scan = id < 6 ? scan4x4 : scan8x8;
        ...
        //-------------------------------------------------------------------------
        static private unsafe void WriteScalingList(ref H264BitWriter bw, int id, byte* scaling_list, int list_size, bool* use_default_matrix)
        //-------------------------------------------------------------------------
        {
            byte[] scan = id < 6 ? scan4x4 : scan8x8;
            ...
            else for (int j = 0; j < list_size; j++)
            {
                byte nextScale = scaling_list[scan[j]];
